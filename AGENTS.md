...
- `pnpm lint` should be clean. It long reported a `react-hooks/static-components` error in `apps/web/components/charts/treemap-highlight.tsx`, which is fixed — the node renderer is declared at module scope instead of inside the chart component.
